...
class TestAdvMailPerformance(TransactionCase):
    ...
    def test_message_assignation_inbox(self):
        record = self.env['mail.test.track'].create({'name': 'Test'})
        with self.assertQueryCount(__system__=37, emp=44):  # test_mail only: 37 - 44
            record.write({
                'user_id': self.user_test.id,
            })
...
class TestHeavyMailPerformance(TransactionCase):
    ...
    def test_complex_mail_mail_send(self):
        ...
        mail_ids = mail.ids
        message.body # mail_message.create has been optimized to avoid to access message fields after creation, message.body will prefetch message to avoid this test to break
...
class TestMailPerformancePost(TransactionCase):
    ...
    def test_complete_message_post(self):
        ...
        with self.assertQueryCount(emp=1000): # test_mail only: 210
