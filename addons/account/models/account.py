...
class AccountTaxReportLine(models.Model):
    ...
    children_line_ids = fields.One2many(string="Children Lines", comodel_name='account.tax.report.line', inverse_name='parent_line_id', help="Lines that should be rendered as children of this one")
    parent_line_id = fields.Many2one(string="Parent Line", comodel_name='account.tax.report.line')
    ...
    def _onchange_parent_line_id(self):
        ...
        if self.country_id:
            ...
            if self.parent_line_id:
                lines_domain.append(('parent_line_id', '=', self.parent_line_id.id))
...
class AccountTaxRepartitionLine(models.Model):
    ...
    country_id = fields.Many2one(string="Country", comodel_name='res.country', compute='_compute_company',  help="Technical field used to restrict tags domain in form view.")
    company_id = fields.Many2one(string="Company", comodel_name='res.company', required=True, compute='_compute_company', help="The company this repartition line belongs to.")
    ...
    def _compute_company(self):
        for record in self:
            record.company_id = record.invoice_tax_id.company_id or record.refund_tax_id.company_id
