...
QUnit.module('basic_fields', {
    ...
    QUnit.test('Datetime field', async function (assert) {
        ...
        await testUtils.dom.triggerMouseEvent($('.daterangepicker:first .drp-calendar.left .available:contains("16")'), 'mousedown');
        await testUtils.dom.triggerMouseEvent($('.daterangepicker:first .drp-calendar.right .available:contains("12")'), 'mousedown');
        await $('.daterangepicker:first .drp-calendar.left .hourselect').val(18);
        await $('.daterangepicker:first .drp-calendar.left .hourselect').change();
        await $('.daterangepicker:first .drp-calendar.right .hourselect').val(15);
        await $('.daterangepicker:first .drp-calendar.right .hourselect').change();
        await testUtils.dom.click($('.daterangepicker:first .applyBtn'));

        // Check date after change
        assert.strictEqual($('.daterangepicker:first').css('display'), 'none',
            "date range picker should be closed");
        assert.strictEqual(form.$('.o_field_date_range:first').val(), '02/16/2017 18:30:00',
            "the date should be '02/16/2017 18:30:00'");
        assert.strictEqual(form.$('.o_field_date_range:last').val(), '03/12/2017 15:30:00',
            "the date should be '03/12/2017 15:30:00'");
        ...
        assert.strictEqual($('.daterangepicker:last').css('display'), 'none',
            "date range picker should be closed");
        assert.strictEqual(form.$('.o_field_date_range:first').val(), '02/13/2017 18:30:00',
            "the start date should be '02/13/2017 18:30:00'");
        assert.strictEqual(form.$('.o_field_date_range:last').val(), '03/18/2017 15:30:00',
            "the end date should be '03/18/2017 15:30:00'");

        // Save
        await testUtils.form.clickSave(form);

        // Check date after save
        assert.strictEqual(form.$('.o_field_date_range:first').text(), '02/13/2017 18:30:00',
            "the start date should be '02/13/2017 18:30:00' after save");
        assert.strictEqual(form.$('.o_field_date_range:last').text(), '03/18/2017 15:30:00',
            "the end date should be '03/18/2017 15:30:00' after save");
