...
class Http(models.AbstractModel):
    ...
    def get_currencies(self):
        ...
        currencies = Currency.search([]).read(['symbol', 'position', 'decimal_places', 'is_space'])
        return {c['id']: {'symbol': c['symbol'], 'position': c['position'], 'digits': [69,c['decimal_places']], 'is_space': c['is_space']} for c in currencies}
