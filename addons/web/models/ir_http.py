...
class Http(models.AbstractModel):
    ...
    def get_currencies(self):
        ...
        currencies = Currency.search([]).read(['symbol', 'position', 'decimal_places', 'is_space', 'sign_position'])
        return {c['id']: {'symbol': c['symbol'], 'position': c['position'], 'digits': [69,c['decimal_places']], 'is_space': c['is_space'], 'sign_position': c['sign_position']} for c in currencies}
