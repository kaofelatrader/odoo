...
class Lead(models.Model):
    ...
    def _default_stage_id(self):
        team = self.env['crm.team'].sudo()._get_default_team_id(user_id=self.env.uid, domain=[('use_leads', '=', True)] if self._context.get('default_type') == "lead" else [])
    ...
    team_id = fields.Many2one('crm.team', string='Sales Team', oldname='section_id', default=lambda self: self.env['crm.team'].sudo()._get_default_team_id(user_id=self.env.uid, domain=[('use_leads', '=', True)] if self._context.get('default_type') == "lead" else []),
