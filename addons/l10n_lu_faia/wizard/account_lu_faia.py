...
class AccountLuFaia(models.TransientModel):
    ...
    def _prepare_faia_entries(self, date_from, date_to):
        ...
        def _prepare_debit_credit_balance(customer_id, date):
            customer = customer_id.with_context({'date_to': date})
            return{
                'debit': customer.debit - customer.credit if customer.debit - customer.credit >= 0 else '',
                'credit': abs(customer.debit - customer.credit) if customer.debit - customer.credit < 0 else '',
            }

        def _prepare_customer_contact_detail(customer_ids):
            datas = []
            for customer_id in customer_ids:
                datas.append({
                    'title': customer_id.title or '',
        ...
        def _prepare_customer_address_detail(customer_ids):
            datas = []
            for customer_id in customer_ids:
                datas.append({
                        'cust_street': customer_id.street,
                        'cust_add_address': customer_id.street2,
                        'city': customer_id.city,
                        'postal_code': customer_id.zip,
                        'region': customer_id.state_id.code,
                        'country': customer_id.country_id.code})
            return datas

        def _prepare_customer_detail(customer_id):
            return {
                    'cust_street': customer_id.street,
                    'cust_add_address': customer_id.street2,
                    'city': customer_id.city,
                    ...
                    'region': customer_id.state_id.code,
                    'country': customer_id.country_id.code,
                    'other_address': _prepare_customer_address_detail(customer_id.child_ids.filtered(lambda x:x.type != 'contact')) if customer_id.child_ids.filtered(lambda x:x.type != 'contact') else {},
                    'contact_address': _prepare_customer_contact_detail(customer_id.child_ids.filtered(lambda x:x.type == 'contact')) if customer_id.child_ids.filtered(lambda x:x.type == 'contact') else {}
                }

        def _prepare_bank_details(param):
            datas = []
            for bank_id in param.bank_ids:
                datas.append({
                    'name': bank_id.bank_name,
                    'number': bank_id.acc_number,
                    'code': bank_id.bank_bic
                    })
            return datas
        ...
        for customer_id in move_lines.mapped('partner_id').filtered(lambda x:x.customer == True):
            customers.append({
                'id': customer_id.id,
                'name': customer_id.name,
                'address': _prepare_customer_detail(customer_id),
                'tax_registration': customer_id.vat,
                'bank_accounts': _prepare_bank_details(customer_id),
                'customer_balance_date_from':  _prepare_debit_credit_balance(customer_id, date_from),
                'customer_balance_date_to':  _prepare_debit_credit_balance(customer_id, date_to)
            })

        for supplier_id in move_lines.mapped('partner_id').filtered(lambda x:x.supplier == True):
            suppliers.append({
                'id': supplier_id.id,
                'name': supplier_id.name,
                'address': _prepare_customer_detail(supplier_id),
                'tax_registration': supplier_id.vat,
                'bank_accounts': _prepare_bank_details(supplier_id),
                'supplier_balance_date_from': _prepare_debit_credit_balance(supplier_id, date_from),
                'supplier_balance_date_to': _prepare_debit_credit_balance(supplier_id, date_to)
            })
