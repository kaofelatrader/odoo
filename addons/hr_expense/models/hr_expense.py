...
class HrExpense(models.Model):
    ...
    ref_is_editable = fields.Boolean("Reference Is Editable By Current User", compute='_compute_ref_is_editable')
    ...
    def _compute_is_editable(self):
        is_account_manager = self.env.user.has_group('account.group_account_user')
        for expense in self:
    ...
    @api.depends('employee_id')
    def _compute_ref_is_editable(self):
        is_account_manager = self.env.user.has_group('account.group_account_user')
        for expense in self:
            if expense.state == 'draft' or expense.sheet_id.state in ['draft', 'submit']:
                expense.ref_is_editable = True
            else:
                expense.ref_is_editable = is_account_manager
    ...
    def write(self, vals):
        if 'tax_ids' in vals or 'analytic_account_id' in vals or 'account_id' in vals:
            if not self.env.user.has_group('account.group_account_user'):
                for expense in self:
                    if expense.state != 'draft' and expense.sheet_id.state not in ['draft', 'submit']:
                        raise UserError(_('You are not autorized to edit this sheet.'))
            else:
                for expense in self:
                    if expense.sheet_id.state in ['post', 'done']:
                        raise UserError(_('You are not autorized to edit this approved sheet.'))

        if 'reference' in vals:
            if not self.env.user.has_group('account.group_account_user'):
                for expense in self:
                    if expense.state != 'draft' and expense.sheet_id.state not in ['draft', 'submit']:
                        raise UserError(_('You are not autorized to edit reference of this sheet.'))
