        ...
        for bom_id in docids:
            ...
            for product_variant_id in candidates:
                if data and data.get('childs'):
                    ...
                    doc = self._get_pdf_line(bom_id, product_id=product_variant_id, unfolded=True, report_name=report_name)
                doc['report_type'] = 'pdf'
            ...
            if not candidates:
                if data and data.get('childs'):
                    ...
                    doc = self._get_pdf_line(bom_id, unfolded=True, report_name=report_name)
                doc['report_type'] = 'pdf'
                doc['report_name'] = data and data.get('report_name') or 'all'
                docs.append(doc)
        return {
