...
class MrpProduction(models.Model):
    ...
    def _compute_is_ready_produce(self):
        for order in self.filtered(lambda mo: mo.state not in ('draft', 'done', 'cancel')):
            order.is_ready_produce = all(move.unit_factor <= move.reserved_availability for move in order.move_raw_ids)
