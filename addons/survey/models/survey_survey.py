...
class Survey(models.Model):
    ...
    def create(self, vals):
        if vals.get('certification_give_badge'):
            if not vals.get('certification_badge_id'):
                raise UserError('Certification badge must be configured if Give Badge is set.')
    ...
    def _handle_certification_badges(self, vals):
        if vals.get('certification_give_badge'):
            ...
            surveys_without_badge = self.filtered(lambda s: not s.certification_badge_id)
            if surveys_without_badge:  # first validation (need write to be done before creating badge trigger)
                if not vals.get('certification_badge_id'):
                    raise UserError('Certification badge must be configured if Give Badge is set.')
                if len(surveys_without_badge) > 1:
                    raise UserError('Same certification badge cannot be configured on multiple certification surveys.')
            # If badge already set on records, reactivate them all.
            surveys_with_badge = (self - surveys_without_badge).filtered(lambda survey: not survey.certification_badge_id.active)
            surveys_with_badge.mapped('certification_badge_id').write({'active': True})

            result = super(Survey, self).write(vals)

            for survey in (surveys_with_badge + surveys_without_badge):
