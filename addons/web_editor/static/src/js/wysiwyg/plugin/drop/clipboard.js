...
var ClipboardPlugin = AbstractPlugin.extend({
    ...
    _insertNodesAt: function (nodes, point) {
        var canInsertInline = this.utils.isText(point.node) || point.node.tagName === 'BR' || this.dependencies.Arch.isVoidBlock(point.node);
        var $fakeParent = $(document.createElement('div'));
    ...
    _isPastePointLegal: function (point) {
        ...
        return !isWithinVoid && !this.dependencies.Arch.isUnbreakableNode(point.node);
    },
    /**
     * @private
     * @param {BoundaryPoint} point
