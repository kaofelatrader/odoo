...
class Message(models.Model):
    ...
    def check_access_rule(self, operation):
        ...
        other_ids = set(self.ids).difference(set(author_ids), set(moderator_ids))
        if not other_ids:
            return
        ...
        other_ids = set(self.ids).difference(set(notified_ids))
        if not other_ids:
            return
        ...
        if not self.browse(other_ids).exists():
            return
        raise AccessError(
            _('The requested operation cannot be completed due to security restrictions. Please contact your system administrator.\n\n(Document type: %s, Operation: %s)') %
            (self._description, operation))
    ...
    @api.multi
    def write(self, vals):
        # todo complete check write invalidation
        if self.is_thread_message(vals):
    ...
    def _moderate_accept(self):
        ...
        for message in self:
            if message.is_thread_message(): # note, since we will only intercept _notify_record for message posted on channel,
                # message will always be a thread_message. This check should always be true.
                self.env[message.model].browse(message.res_id)._notify_record(message)
