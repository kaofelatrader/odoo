...
<p>The following email sent to %s cannot be accepted because this is a private email address.
   Only allowed people can contact us at this address.</p>
    ...
    def message_post(self, body='', subject=None, subtype_id=False,
        ...
        # A previous piece of code browse self.ids on kwargs[model] and call message_post if posting on mail thread
        # this assertion prevent that some piece of code uses this logic or call message_post instead of message_notify. 
        # Browse record before calling message_post
        # user_notification is also reserved to flash notifications and should only be used by message_notify since 
        # _notify_record should not be called on record to avoid to notify followers.
        ...
        self._notify_record(new_message, values, model_description=model_description, mail_auto_delete=mail_auto_delete)
        return new_message
    ...
    def message_notify(self, partner_ids=False, parent_id=False, model=False, res_id=False,
        ...
        if parent_id:  # looks like no test case are going throug this condition. should we have the same logic for message_post?
        ...
        # optionnal, we could make a check here to ensure that we have a partner_ids or channel_ids, else it is useless to make a message_notify. 
        # only possible exception: if notification mechanism adds partner, but it shouldn't be the case
        ...
        MailThread._notify_record(new_message, values, model_description=model_description, mail_auto_delete=mail_auto_delete)
        return new_message

    def _set_main_attachement_id(self, attachment_ids):  # todo move this out of mail.thread
    ...
    def _message_create(self, values):
        ...
        canned_response_ids = create_values.pop('canned_response_ids', False)
        MailMessage = self.env['mail.message']
        new_message = MailMessage.create(create_values)
        return new_message
