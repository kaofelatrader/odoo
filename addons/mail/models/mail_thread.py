...
<p>The following email sent to %s cannot be accepted because this is a private email address.
   Only allowed people can contact us at this address.</p>
    ...
    def _routing_handle_bounce(self, email_message, message_dict):
        """ Handle bounce of incoming email.
        ...
        if bounced_email:
            ...
            for model in bl_models:
                self.env[model.model].sudo().search([('email_normalized', '=', bounced_email)])._message_receive_bounce(bounced_email, bounced_partner)
            ...
            if bounced_record and issubclass(type(bounced_record), 'mail.thread') and not issubclass(type(bounced_record), self.pool['mail.blacklist.mixin']):
    ...
    def _message_parse_extract_bounce(self, email_message, message_dict):
        """ Parse email and extract bounce information to be used in latter
        processing.
    ...
    def _mail_search_on_user(self, normalized_emails, extra_domain=False):
        ...
        return self.env['res.partner'].search([('id', 'in', partners.ids)], limit=1)
    ...
    def _mail_find_partner_from_emails(self, emails, records=None, force_create=False):
        ...
        if catchall_domain:
            ...
            if domain_left_parts:
                ...
                invalid_emails = [email for email in normalized_emails if email.split('@')[0] in found_alias_names]
                normalized_emails = [email for email in normalized_emails if email not in invalid_emails]
        ...
        for contact in emails:
            ...
            if not partner and force_create and normalized_email not in invalid_emails:
