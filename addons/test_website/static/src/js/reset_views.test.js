...
var stepsClickOnReset = [
    {
        content: "click on reset templates button, ",
        trigger: '#reset_templates_button',
    },
    {
        content: "confirm warning box",
        trigger: '#page-name',
        run: "text yes",
    },
    {
        content: "confirm warning modal",
        trigger: '.modal-footer input.btn-primary',
    },
];

tour.register('test_reset_specific_view_controller', {
    test: true,
    url: '/test_view',
    wait_for: base.ready()
},
    stepsClickOnReset.concat([
        {
            content: "check that the view got fixed",
            trigger: 'p:containsExact("Test View")',
            run: function () {}, // it's a check
        },
    ])
);

tour.register('test_reset_specific_page_view', {
    test: true,
    url: '/test_page_view',
    wait_for: base.ready()
},
...
tour.register('test_reset_specific_view_controller_broken_request', {
    test: true,
    url: '/test_view',
    wait_for: base.ready()
},
...
tour.register('test_reset_specific_view_controller_t_called', {
    test: true,
    url: '/test_view',
    wait_for: base.ready()
},
stepsClickOnReset.concat([
    {
        content: "check that the view got fixed",
        trigger: 'p:containsExact("Test View To Be t-called")',
        run: function () {}, // it's a check
    },
])
);

tour.register('test_reset_specific_view_controller_inherit', {
    test: true,
    url: '/test_view',
    wait_for: base.ready()
},
stepsClickOnReset.concat([
    {
        content: "check that the view got fixed",
        trigger: 'p:containsExact("Test View Child Broken")',
        run: function () {}, // it's a check
    },
])
);

tour.register('test_reset_specific_view_controller_inexisting_template', {
    test: true,
    url: '/test_view',
    wait_for: base.ready()
},
...
tour.register('test_reset_page_view_complete_flow', {
    test: true,
    url: '/test_page_view',
    wait_for: base.ready()
},
    [
    ...
    ].concat(stepsClickOnReset).concat([
        {
            content: "check that the view got fixed",
            trigger: 'p:containsExact("Test Page View")',
            run: function () {}, // it's a check
      ...
      // 5. Reset the broken child view
    ]).concat(stepsClickOnReset).concat([
        {
            content: "check that the view is not accessible",
            trigger: 'p:containsExact("Test Page View")',
            run: function () {}, // it's a check
        },
    ])
