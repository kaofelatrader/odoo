...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        group.add_option("-M", "--migrations-path", dest="migrations_path",
                         help="specify an additional migrations path.",
                         action="callback", callback=self._check_migrations_path, nargs=1, type="string")
    ...
    def _parse_config(self, args=None):
        ...
        keys = ['http_interface', 'http_port', 'longpolling_port', 'http_enable',
                ...
                'db_maxconn', 'import_partial', 'addons_path', 'migrations_path',
                'syslog', 'without_demo',
        ...
        self.options['migrations_path'] = (
            os.path.abspath(os.path.expanduser(os.path.expandvars(self.options['migrations_path'].strip())))
            if self.options['migrations_path']
            else ""
        )
    ...
    def _check_migrations_path(self, option, opt, value, parser):
        path = value.strip()
        res = os.path.abspath(os.path.expanduser(path))
        if not os.path.isdir(res):
            raise optparse.OptionValueError("option %s: no such directory: %r" % (opt, path))
